...
export class NewsService {
  constructor(private readonly newsStore: NewsStore, private readonly newsHttpService: NewsHttpService) { }
  ...
  create(title: string, description: string): Observable<New> {
    ...
    return this.newsHttpService.create({ newsUuid: uuid, title, description }).pipe(
      setLoading(this.newsStore),
      map(response => response.news),
      tap(entity => this.newsStore.add(entity, { prepend: true })),
    );
  }

  update(uuid: string, title: string, description: string): Observable<New> {
